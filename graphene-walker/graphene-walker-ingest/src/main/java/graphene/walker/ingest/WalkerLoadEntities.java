 ...
 * 
 */
public class WalkerLoadEntities {
	...
	public static final String tableSource = "WalkerGraph.txt";

	private void parseEmail(String email, Integer id) throws SQLException {
		
		// String email might be a comma-separated list of email address
		String[] emails = email.split(",");
		
		//TODO: remove duplicates from emails[]?
		
		for (int i = 0; i < emails.length; i++) {
			String name = emailToName(emails[i]);
			if (name == null) return;
			
			// The name we reconstructed.
			ps.setInt(1, id); // customernumber equiv
			ps.setString(2, emails[i]); // use email address as the id
			...
			ps.setInt(3, App.eId++); // primary key for table
			ps.setString(4, emails[i]);// identifier
			ps.setString(5, tableSource);
